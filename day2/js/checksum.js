...
console.log('diff sum:', getDiffSum(input));
console.log('div sum:', getDivisibleSum(input));

function getDiffSum(input) {
	let diffSum = 0;
	input.forEach((line) => {
		const lineArr = line.split(/\t/).map((n) => {
			return parseInt(n, 10);
		});
		diffSum += Math.max(...lineArr) - Math.min(...lineArr);
	});
	return diffSum;
}

function getDivisibleSum(input) {
	let divSum = 0;
	input.forEach((line) => {
		const lineArr = line.split(/\t/).map((n) => {
			return parseInt(n, 10);
		...
		for (let i = 0; i < lineArr.length; i++) {
			const currentValue = lineArr[i];
			for (let j = 0; j < lineArr.length; j++) {
				if (lineArr[i] !== lineArr[j] && lineArr[i] % lineArr[j] === 0) {
					divSum += lineArr[i] / lineArr[j];
				}
			}
		}
	});
